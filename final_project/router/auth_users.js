...
regd_users.put("/auth/review/:isbn", (req, res) => {
  ...
  return res.status(200).json({ message: `Review for book with ISBN ${isbn} by user ${username} has been added/updated.` });
});
...
regd_users.delete("/auth/review/:isbn", (req, res) => {
  ...
  if (books[isbn]) {
    ...
    return res.status(404).json({ message: `User ${username} has not posted a review for this book.` });
  }
  return res.status(404).json({ message: "Book not found." });
});
